...
if ( ! defined( 'MNSK7_THEME_VERSION' ) ) {
	define( 'MNSK7_THEME_VERSION', '1.0.94' );
}
...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_enqueue_script( 'mnsk7-header', get_stylesheet_directory_uri() . '/assets/js/header.js', array(), $v, true );
	wp_localize_script(
		'mnsk7-header',
		'mnsk7HeaderConfig',
		array(
			'viewAllPrefix'  => __( 'Zobacz wszystkie produkty w', 'mnsk7-storefront' ),
			'subcategoryOne' => _x( 'podkategoria', 'megamenu', 'mnsk7-storefront' ),
			'subcategoryFew' => _x( 'podkategorie', 'megamenu', 'mnsk7-storefront' ),
			'subcategoryMany' => _x( 'podkategorii', 'megamenu', 'mnsk7-storefront' ),
		)
	);
	...
	if ( is_front_page() ) {
		...
		wp_enqueue_script( 'mnsk7-bestsellers-a11y', get_stylesheet_directory_uri() . '/assets/js/bestsellers-a11y.js', array(), $v, true );
	}
	if ( is_singular( 'product' ) ) {
		wp_enqueue_script( 'mnsk7-pdp-gallery-a11y', get_stylesheet_directory_uri() . '/assets/js/pdp-gallery-a11y.js', array(), $v, true );
		wp_enqueue_script( 'mnsk7-pdp-gallery', get_stylesheet_directory_uri() . '/assets/js/pdp-gallery.js', array(), $v, true );
	}
	if ( function_exists( 'is_shop' ) && ( is_shop() || is_product_category() || is_product_tag() ) ) {
...
add_action( 'wp_footer', function () {
	// Legacy inline controller is intentionally disabled. Header runtime lives in assets/js/header.js.
	return;
	...
	(function() {
		...
		function isSearchPanelViewport() {
			return window.innerWidth < DESKTOP_MIN;
		}
		...
		function syncSearchPresentation() {
			...
			if (searchToggle) {
				var expanded = isSearchPanelViewport() && mobileOpen;
		...
		function closeSearch() {
			if (!isSearchPanelViewport()) return;
		...
		if (menuToggle && nav) {
			...
			// Tap na przyciemnienie / treść PDP (poza drawerem) zamyka menu.
			document.addEventListener('click', function(e) {
				if (window.innerWidth >= DESKTOP_MIN || !nav.classList.contains('is-open')) return;
				var t = e.target;
				if (menuToggle && (t === menuToggle || menuToggle.contains(t))) return;
				if (menu && menu.contains(t)) return;
				if (t.closest && t.closest('.mnsk7-drawer__l2-panel, .mnsk7-drawer__chrome, .sub-menu.mnsk7-megamenu.mnsk7-drawer__panel--open')) return;
				closeMenu();
		...
		// Mobile (<=1023): prosty drawer-akordeon (init niżej). Desktop: hover megamenu.
		if (menu) {
			...
			if (megamenuLi) {
				...
				function setMegamenuExpanded(open) {
					...
					if (open && typeof window.mnsk7MegamenuActivateDefault === 'function') { window.mnsk7MegamenuActivateDefault(); }
				}
			...
			// === Desktop: dwupanelowy megamenu (lewy rail kategorii → prawy panel podkategorii) ===
			(function initDesktopTwoPane() {
				var panel = menu.querySelector('.sub-menu.mnsk7-megamenu');
				if (!panel) return;
				var cards = panel.querySelector('.mnsk7-megamenu__cards');
				if (!cards) return;
				...
				cols.forEach(function(col) {
					...
					if (title && title.tagName !== 'A' && !title.hasAttribute('tabindex')) {
						title.setAttribute('tabindex', '0');
					}
				});

				// Wspólny element CTA dla kategorii-liści (bez podkategorii) — tylko desktop.
				var cta = cards.querySelector('.mnsk7-megamenu__cta');
				if (!cta) {
					cta = document.createElement('div');
					cta.className = 'mnsk7-megamenu__cta';
					cta.setAttribute('hidden', '');
					var a = document.createElement('a');
					a.className = 'mnsk7-megamenu__cta-link';
					a.setAttribute('href', '#');
					cta.appendChild(a);
				...
				function activate(col) {
					if (!col) return;
					cols.forEach(function(c) { if (c !== col) c.classList.remove('is-active'); });
				...
				// Domyślnie aktywuj pierwszą kategorię z podkategoriami przy otwarciu panelu (desktop).
				window.mnsk7MegamenuActivateDefault = function() {
					if (window.innerWidth < DESKTOP_MIN) return;
					if (cards.querySelector('.mnsk7-megamenu__col.is-active')) return;
					var firstWithList = null;
					for (var k = 0; k < cols.length; k++) {
						if (cols[k].querySelector(':scope > .mnsk7-megamenu__list')) { firstWithList = cols[k]; break; }
					}
					activate(firstWithList || cols[0]);
				};

				// Klawiatura: ↑/↓ po railu, → wchodzi w prawy panel, ← wraca do railu.
				cards.addEventListener('keydown', function(e) {
					if (window.innerWidth < DESKTOP_MIN) return;
					var titles = cols.map(function(c) { return c.querySelector(':scope > .mnsk7-megamenu__col-title'); }).filter(Boolean);
					var idx = titles.indexOf(document.activeElement);
					if (e.key === 'ArrowDown') {
						if (idx > -1) { e.preventDefault(); titles[(idx + 1) % titles.length].focus(); }
					} else if (e.key === 'ArrowUp') {
						if (idx > -1) { e.preventDefault(); titles[(idx - 1 + titles.length) % titles.length].focus(); }
					} else if (e.key === 'ArrowRight') {
						var active = cards.querySelector('.mnsk7-megamenu__col.is-active');
						var target = active && active.querySelector('.mnsk7-megamenu__list a');
						if (!target && cta && !cta.hasAttribute('hidden')) { target = ctaLink; }
						if (target) { e.preventDefault(); target.focus(); }
					} else if (e.key === 'ArrowLeft') {
						var act = cards.querySelector('.mnsk7-megamenu__col.is-active');
						var t = act && act.querySelector(':scope > .mnsk7-megamenu__col-title');
						if (t) { e.preventDefault(); t.focus(); }
					}
				});
			})();

			// === Mobile drawer: prosty akordeon, bez odpinanych paneli i wielopoziomowych animacji ===
			(function initMobileDrawer() {
				...
				var EXPAND_LABEL = <?php echo wp_json_encode( __( 'Rozwiń', 'mnsk7-storefront' ) ); ?>;
				var COLLAPSE_LABEL = <?php echo wp_json_encode( __( 'Zwiń', 'mnsk7-storefront' ) ); ?>;
				var toggleIndex = 0;
				...
				// Stały pasek drawera: tytuł + jedno, zawsze widoczne zamknięcie.
				var chrome = menu.querySelector('.mnsk7-drawer__chrome');
				if (!chrome) {
					chrome = document.createElement('li');
					chrome.className = 'mnsk7-drawer__chrome mnsk7-drawer__chrome--simple';
				...
				chrome.classList.add('mnsk7-drawer__chrome--simple');
				var oldBackBtn = chrome.querySelector('.mnsk7-drawer__back');
				if (oldBackBtn) oldBackBtn.remove();
				var titleEl = chrome.querySelector('.mnsk7-drawer__title');
				var closeBtn = chrome.querySelector('.mnsk7-drawer__close');
				if (titleEl) titleEl.textContent = ROOT_TITLE;

				function setExpanded(owner, button, expanded) {
					owner.classList.toggle('is-mobile-expanded', expanded);
					button.setAttribute('aria-expanded', expanded ? 'true' : 'false');
					var label = button.getAttribute('data-mnsk7-label') || '';
					button.setAttribute('aria-label', (expanded ? COLLAPSE_LABEL : EXPAND_LABEL) + ': ' + label);
				}

				function addAccordionToggle(owner, anchor, panel) {
					if (!owner || !anchor || !panel || owner.querySelector(':scope > .mnsk7-drawer__submenu-toggle')) return;
					toggleIndex++;
					if (!panel.id) panel.id = 'mnsk7-mobile-submenu-' + toggleIndex;
					var label = drawerRowLabel(anchor);
					var button = document.createElement('button');
					button.type = 'button';
					button.className = 'mnsk7-drawer__submenu-toggle';
					button.setAttribute('aria-controls', panel.id);
					button.setAttribute('data-mnsk7-label', label);
					button.innerHTML = '<span aria-hidden="true"></span>';
					anchor.insertAdjacentElement('afterend', button);
					owner.classList.add('mnsk7-drawer__accordion-item');
					setExpanded(owner, button, false);
					button.addEventListener('click', function(e) {
						e.preventDefault();
						e.stopPropagation();
						setExpanded(owner, button, button.getAttribute('aria-expanded') !== 'true');
					});
				}

				var shopItem = menu.querySelector(':scope > li.menu-item-has-children');
				...
				menu.querySelectorAll('.mnsk7-megamenu__col').forEach(function(col) {
					var list = col.querySelector(':scope > .mnsk7-megamenu__list');
					var title = col.querySelector(':scope > .mnsk7-megamenu__col-title');
					addAccordionToggle(col, title, list);
				});

				function resetAccordions() {
					menu.querySelectorAll('.mnsk7-drawer__accordion-item').forEach(function(item) {
						var button = item.querySelector(':scope > .mnsk7-drawer__submenu-toggle');
						if (button) setExpanded(item, button, false);
					});
				}
				...
				window.mnsk7DrawerReset = resetAccordions;
				window.mnsk7DrawerOnOpen = function() {
					...
					setTimeout(function() {
						...
						else if (closeBtn) { try { closeBtn.focus(); } catch (e) {} }
					}, 40);
				};
			})();
		}
...
add_action( 'wp_footer', function () {
	...
	(function() {
		...
		function enhanceMobileFallbackFilterRows() {
			...
			if (document.querySelector('.mnsk7-plp-dropdown')) return;
			var rows = document.querySelectorAll('.mnsk7-plp-chips--nav, .mnsk7-plp-chips--attrs');
			rows.forEach(function(row, index) {
				if (row.closest('.mnsk7-plp-dropdown') || row.dataset.mnsk7MobileModal === '1') return;
				var labelEl = row.querySelector('.mnsk7-plp-chips__label');
				var label = labelEl ? (labelEl.textContent || '').trim() : (index === 0 ? 'Filtry' : 'Opcje');
				var active = row.querySelector('.mnsk7-plp-chip--active, [aria-current="page"]');
				var meta = active ? ('Wybrano: ' + (active.textContent || '').trim().replace(/\s+/g, ' ')) : 'Dotknij, aby wybrać';
...
function mnsk7_render_trust_badges( $wrapper_class = 'mnsk7-pdp-trust' ) {
	$eta_label = function_exists( 'mnsk7_delivery_eta_badge_label' ) ? mnsk7_delivery_eta_badge_label() : __( 'Wysyłka jutro', 'mnsk7-storefront' );
